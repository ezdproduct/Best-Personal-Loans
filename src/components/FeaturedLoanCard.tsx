...
const FeaturedLoanCard: React.FC<FeaturedLoanCardProps> = ({ provider }) => {
  const features = [
  ...
  return (
    <div className="bg-white rounded-lg shadow-xl overflow-hidden border-4 border-teal-500 relative pt-8">
      {/* Header Tab */}
      <div className="absolute top-0 left-0 bg-teal-600 text-white px-4 py-1 rounded-br-lg flex items-center space-x-3">
        <span className="flex items-center justify-center w-6 h-6 rounded-full bg-white text-teal-600 font-bold text-sm">1</span>
        <span className="font-semibold text-sm">Our Top Pick</span>
      </div>
